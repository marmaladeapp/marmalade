class App::Projects::Finances::ReceivablesController < App::AppController
  ...
  def destroy
    if params[:resource_id]
      ...
      #balance_sheets
      @ledger.destroy
      redirect_to resource_project_receivables_path(@resource,@project)
    elsif params[:user_id]
      @user = User.find(params[:user_id])
      @resource = @user.home
      @context = @resource
      ...
      #balance_sheets
      @ledger.destroy
      redirect_to user_home_project_receivables_path(@user,@project)
    elsif params[:group_id]
      @resource = Group.find(params[:group_id])
      @context = @resource
      authorize! :show, @context, :message => ""
      ...
      #balance_sheets
      @ledger.destroy
      redirect_to group_project_receivables_path(@resource,@project)
    end
  end
