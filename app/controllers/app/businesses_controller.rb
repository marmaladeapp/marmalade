class App::BusinessesController < App::AppController
  ...
  def create
    ...
    @business.subscriber_id = @user.id
    if @business.save
  ...
  def business_params
    params.require(:business).permit(:name,:description,:subscriber_id,:currency,:country,:time_zone,:owners_attributes => [:global_owner,:equity])
  end
end
